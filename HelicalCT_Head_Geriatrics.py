...
def get_spectrum(n):
    ...
    path = '/home/davlars/GPUMCI_HeadSimulations/'
    dat = np.loadtxt(os.path.join(path,'spectrumGeriatrics.txt'))
...
def getPhantom(phantomName):
    ...
    nii = nib.load(os.path.joint(path,phantomName))
    phantomdata = nii.get_data()
                          
    densities = np.zeros_like(phantomdata, dtype=float)
    densities[phantomdata == 0] = 0   # air
    densities[phantomdata == 1] = 1.015   # fat
    densities[phantomdata == 2] = 1.8   # bone
    densities[phantomdata == 3] = 1.041 # grey matter
    densities[phantomdata == 4] = 1.0315 # white matter
...
def calculate_projections(phantomName, saveName, turnNumber):
    ...
    materials = ['air', 'water', 'bone', 'grey_matter', 'white_matter']
...
if __name__ == '__main__':
    ...
    folder = '/my/output/70100644Phantom_labelled_no_bed'
    ...
    for phantomName in names[:1]:
        for number_of_simu in range(gpuindex, nsimul, ngpus):
            for turnNumber in range(23):
                ...
                saveName = ('{}/helical_proj_120kVSpectrum_{}'.format(folder, phantomName)[:-4] + 
